...
list<list<list<MatrixXd>>> spinInversionHamiltonian2(double J_ratio, int N) {
                    ...
                    for (int z : {-1, 1}) {
                        ...
                        for (int s : s_vector_m) {
                            for (int sigma : {-1, 1}) {
                                ...
                                vector<int> R_mp_mz_mpz = checkState_inversion2(s, k, N);
                                int m = -1;
                                int n = -1;
                                int c = getStateCase2(R_mp_mz_mpz[1], R_mp_mz_mpz[2], R_mp_mz_mpz[3]);

                                if (c == 2) {
                                    m = R_mp_mz_mpz[1];
                                    double Na = N_a_sigma_inv2(N, sigma * R_mp_mz_mpz[0], k, p, z, m, n, c);
                                    double Na_min = N_a_sigma_inv2(N, -sigma * R_mp_mz_mpz[0], k, p, z, m, n, c);
                        ...
                        for (int a = 0; a < K; a++) {
                            ...
                            setHElementsForState_inversion2(N, k, p, z, s_vector_k, R_vector, m_vector, n_vector, c_vector, K, H, a, s, n, 1, 1);
                            setHElementsForState_inversion2(N, k, p, z, s_vector_k, R_vector, m_vector, n_vector, c_vector, K, H, a, s, n, 2, J_ratio);
                        }
...
void setHElementsForState_inversion2(const int N, const int k, const int p, const int z, const vector<int> & s_vec,
    ...
    for (int i = 0; i < N; i++) {
        ...
        if (getBit(s_prime, i) != getBit(s_prime, j)) {
            flipBit(s_prime, i);
            flipBit(s_prime, j);
            vector<int> r_l_q_g = representative_inversion2(s_prime, N);
            ...
            if (b >= 0) {
                ...
                for (int i_mat = a; i_mat < a + n; i_mat++) {
                    for (int j_mat = b; j_mat < b + m; j_mat++) {
                        double val = hElement_inversion2(i_mat, j_mat, r_l_q_g[1], r_l_q_g[2], r_l_q_g[3],
...
double hElement_inversion2(const int a, const int b, const double l, const double q, const double g, const double k, const double p, const double z, const double N,
                          const vector<int> & R_vec, const vector<int> & m_vec, const vector<int> & n_vec,
                          const vector<int> & c_vec) {
    double sigma_a = (double) R_vec[a] / (double) abs(R_vec[a]);
    double sigma_b = (double) R_vec[b] / (double) abs(R_vec[b]);
    double Na = N_a_sigma_inv2(N, R_vec[a], k, p, z, m_vec[a], n_vec[a], c_vec[a]);
    double Nb = N_a_sigma_inv2(N, R_vec[b], k, p, z, m_vec[b], n_vec[b], c_vec[b]);
...
double N_a_sigma_inv2(const double N, const double sigmaR, const double k, const double p, const double z, const double m, const double n, const int c) {
...
vector<int> checkState_inversion2(const int s, const int k, const int N) {
    int R = -1;
    int m_p = -1;
    int m_z = -1;
    int m_pz = -1;
...
vector<int> representative_inversion2(const int s, const int N) {
    int l = 0;
    int q = 0;
    int g = 0;
