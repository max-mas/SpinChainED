...
SparseMatrix<complex<double>> spinOp2_momentum_sparse_large_N(int N) {
        ...
        for (int k = -trunc((N+2)/4) + 1 ; k <= trunc(N/4); k++ ) {
            ...
            for (int l = 0; l < K; l++) {
                ...
                for (int i = 0; i < N; i++) {
                    for (int j = 0; j < i; j++) {
                        if (getBit(a, i) == getBit(a, j)) {
                            H.coeffRef(l + currentMatRootIndex, l + currentMatRootIndex) += 0.5;
                        } else {
                            H.coeffRef(l + currentMatRootIndex, l + currentMatRootIndex) += -0.5;
                            ...
                            if (f >= 0) {
                                complex<double> offDiagEl = sqrt((double)R_vector[l] / (double) R_vector[f])
                                                            * std::exp(complex<double>(0,4.0 * M_PI * (double) k * (double) r_L[1] / (double) N));
                                H.coeffRef(l + currentMatRootIndex, f + currentMatRootIndex) += offDiagEl;
                            }
                        }
                    }
                }
...
SparseMatrix<complex<double>> spinOpS2_momentum_sparse_m(int N, int n_up) {
    ...
    __int128 siz = fact(N) / (fact(N-n_up) * fact(n_up));
